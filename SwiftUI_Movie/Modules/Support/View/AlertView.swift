...
struct FetchNoMoreFooter: View {

    var body: some View {
        HStack(alignment: .center) {
            Spacer()
            ZStack {
                Text("沒有更多數據").foregroundColor(.secondary)
            }
            Spacer()
        }
        .frame(height: 50)
...
struct ErrorView: View {
    ...
    var body: some View {
        AlertView(symbol: error.symbol, message: "暫時無資訊") {
